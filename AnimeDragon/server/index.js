...
async function syncAniList(d, progress=()=>{}){
  ...
  const perPage=50;
  // AniList Page has a hard limit of 5000 entries (page * perPage).
  // We therefore use one bounded popular catalog pass instead of walking every year.
  const maxPages=Math.max(1,Math.min(100,Number(d.settings?.anilistMaxPages)||20));
  const currentYear=new Date().getUTCFullYear();
  const currentYearPages=Math.max(1,Math.min(20,Number(d.settings?.anilistCurrentYearPages)||10));
  let discovered=0,updated=0,episodesCreated=0,pages=0;
  const seen=new Set();
  const totalPages=currentYearPages+maxPages;
  progress({currentSource:1,totalSources:1,currentPage:0,totalPages,message:'AniList: iniciando catálogo real…'});

  const fetchPage=async(page,year=null)=>{
  ...
  const passes=[{label:'ano '+currentYear,limit:currentYearPages,year:currentYear},{label:'catálogo geral',limit:maxPages,year:null}];
  for(const pass of passes){
    for(let page=1;page<=pass.limit;page++){
      pages++; progress({currentSource:1,totalSources:1,currentPage:pages,totalPages,message:'AniList · '+pass.label+' · página '+page+'/'+pass.limit});
      const pg=await fetchPage(page,pass.year);
    ...
    write(d);
      if(!pg?.pageInfo?.hasNextPage||!media.length)break;
    }
  }
...
async function syncJikanFallback(d, progress=()=>{}){
  const maxPages=Math.max(1,Math.min(40,Number(d.settings?.jikanMaxPages)||20));
  let discovered=0,updated=0,pages=0;
  const seen=new Set();
  progress({currentSource:2,totalSources:2,currentPage:0,totalPages:maxPages,message:'Jikan: usando catálogo alternativo…'});
  for(let page=1;page<=maxPages;page++){
    progress({currentSource:2,totalSources:2,currentPage:page,totalPages:maxPages,message:'Jikan · página '+page+'/'+maxPages});
    const r=await fetch(`https://api.jikan.moe/v4/anime?page=${page}&limit=25&sfw=true&order_by=popularity&sort=desc`,{headers:{accept:'application/json'},signal:AbortSignal.timeout(20000)});
    if(!r.ok)throw new Error('Jikan HTTP '+r.status);
    const j=await r.json(); const items=Array.isArray(j.data)?j.data:[];
    if(!items.length)break;
    for(const m of items){
      const malId=Number(m.mal_id)||0; if(!malId||seen.has(malId))continue; seen.add(malId);
      const title=String(m.title||m.title_english||m.title_japanese||'').trim(); if(!title)continue;
      let a=d.anime.find(x=>Number(x.malId)===malId||x.slug===slugify(title));
      const genres=[...(m.genres||[]),...(m.themes||[])].map(x=>x?.name).filter(Boolean).slice(0,30);
      const score=Math.max(0,Math.min(10,Number(m.score||0)));
      const statusMap={'Currently Airing':'Ongoing','Finished Airing':'Finished','Not yet aired':'Upcoming'};
      const payload={title,slug:slugify(title),synopsis:String(m.synopsis||'').replace(/<[^>]+>/g,'').slice(0,3000),cover:cleanUrl(m.images?.jpg?.large_image_url||m.images?.jpg?.image_url),banner:cleanUrl(m.images?.jpg?.large_image_url),year:Number(m.year||m.aired?.from?.slice?.(0,4))||null,status:statusMap[m.status]||String(m.status||'Ongoing'),score,type:m.type==='Movie'?'Movie':String(m.type||'TV'),episodes:Number(m.episodes||0),featured:score>=8.5,categories:d.categories.filter(c=>genres.some(t=>slugify(t)===c.slug)).map(c=>c.id),views:a?.views||0,tags:genres,malId,anilistId:a?.anilistId||null,anilistUrl:a?.anilistUrl||'',duration:Number(m.duration?.match?.(/\d+/)?.[0]||0),createdAt:a?.createdAt||now(),updatedAt:now()};
      if(!a){a={id:Math.max(0,...d.anime.map(x=>Number(x.id)||0))+1,...payload};d.anime.push(a);discovered++;}else{Object.assign(a,payload);updated++;}
    }
    write(d);
    if(!j.pagination?.has_next_page)break;
  }
  d.settings.jikanSyncedAt=now(); d.settings.jikanLastResult={discovered,updated,pages,at:now()}; write(d);
  return {source:'Jikan',discovered,updated,episodesCreated:0,pages,at:now()};
}
...
async function startCatalogSync(d){
  ...
  syncJob={id:crypto.randomUUID(),status:'running',startedAt:now(),finishedAt:null,totalSources:2+ext,currentSource:0,currentPage:0,totalPages:0,discovered:0,updated:0,episodesCreated:0,errors:[],message:'Preparando sincronização…'};
  const jobId=syncJob.id;
  setImmediate(async()=>{
    try{
      let discovered=0,updated=0,episodesCreated=0; const errors=[];
      try{
        const r1=await syncAniList(d,p=>{if(syncJob.id===jobId)Object.assign(syncJob,{...p,totalSources:2+ext});});
        discovered+=r1.discovered; updated+=r1.updated; episodesCreated+=r1.episodesCreated;
      }catch(e){
        errors.push({source:'AniList',error:e.message});
        if(syncJob.id===jobId)Object.assign(syncJob,{currentSource:1,message:'AniList indisponível — ativando fonte alternativa Jikan…',errors:[...errors]});
        try{
          const rFallback=await syncJikanFallback(d,p=>{if(syncJob.id===jobId)Object.assign(syncJob,{...p,totalSources:2+ext,discovered,updated,episodesCreated,errors:[...errors]});});
          discovered+=rFallback.discovered; updated+=rFallback.updated;
        }catch(e2){errors.push({source:'Jikan',error:e2.message});}
      }
      try{
        const r2=await syncExternalApis(d,p=>{if(syncJob.id===jobId)Object.assign(syncJob,{...p,currentSource:(p.currentSource||0)+2,totalSources:2+ext,discovered,updated,episodesCreated,errors:[...errors]});});
        discovered+=r2.discovered; updated+=r2.updated; episodesCreated+=r2.episodesCreated; errors.push(...(r2.errors||[]));
      }catch(e){errors.push({source:'APIs externas',error:e.message});}
      d.settings.externalApisSyncedAt=now(); d.settings.externalApisLastResult={discovered,updated,episodesCreated,errors,at:now()}; write(d);
      const usable=discovered||updated||episodesCreated||!errors.length;
      Object.assign(syncJob,{status:usable?'completed':'failed',finishedAt:now(),currentSource:2+ext,discovered,updated,episodesCreated,errors,message:usable?(errors.length?'Concluído com avisos':'Sincronização concluída'):'Nenhuma fonte conseguiu responder'});
...
server.listen(PORT,()=>{
  ...
  if(process.env.AUTO_SYNC !== 'false'){
    // Primeira sincronização imediatamente: não deixe o catálogo vazio por 6 horas.
    setImmediate(()=>{const d=read(); if(!syncRunning) startCatalogSync(d).catch(e=>console.error('initial auto sync',e));});
    // Depois mantém o catálogo atualizado a cada 6 horas.
    setInterval(()=>{const d=read(); if(!syncRunning) startCatalogSync(d).catch(e=>console.error('auto sync',e));},6*60*60*1000);
  }
});
