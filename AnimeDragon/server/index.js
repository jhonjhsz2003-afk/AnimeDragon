...
const demoAnime=[
 {id:1,title:'Dragonfall: Awakening',slug:'dragonfall-awakening',synopsis:'Uma antiga energia dracônica desperta e arrasta um jovem para uma guerra entre reinos, magia e criaturas lendárias.',cover:'https://images.unsplash.com/photo-1518709268805-4e9042af9f23?auto=format&fit=crop&w=800&q=85',banner:'https://images.unsplash.com/photo-1519608487953-e999c86e7455?auto=format&fit=crop&w=1800&q=85',year:2026,status:'Ongoing',score:9.4,type:'TV',episodes:24,featured:true,categories:[1,2,4,6],views:18432,createdAt:'2026-09-08T00:00:00.000Z'},
 {id:2,title:'Crimson Ronin',slug:'crimson-ronin',synopsis:'Em uma cidade neon, um ronin marcado pelo passado recebe uma missão capaz de mudar toda a ordem do submundo.',cover:'https://images.unsplash.com/photo-1518709594023-6eab9bab7b23?auto=format&fit=crop&w=800&q=85',banner:'https://images.unsplash.com/photo-1519608487953-e999c86e7455?auto=format&fit=crop&w=1800&q=85',year:2025,status:'Finished',score:9.1,type:'TV',episodes:12,featured:true,categories:[1,5,6],views:14291,createdAt:'2026-09-06T00:00:00.000Z'},
 {id:3,title:'Moonlit Academy',slug:'moonlit-academy',synopsis:'Uma academia secreta treina jovens capazes de controlar magia lunar antes que uma guerra esquecida retorne.',cover:'https://images.unsplash.com/photo-1516339901601-2e1b62dc0c45?auto=format&fit=crop&w=800&q=85',banner:'https://images.unsplash.com/photo-1469474968028-56623f02e42e?auto=format&fit=crop&w=1800&q=85',year:2026,status:'Ongoing',score:8.9,type:'TV',episodes:10,featured:true,categories:[4,5,7],views:11202,createdAt:'2026-09-05T00:00:00.000Z'},
 {id:4,title:'Neon Circuit',slug:'neon-circuit',synopsis:'Pilotos rivais disputam a corrida mais perigosa do futuro, onde mechas e tecnologia proibida valem mais que qualquer prêmio.',cover:'https://images.unsplash.com/photo-1535378917042-10a22c95931a?auto=format&fit=crop&w=800&q=85',banner:'https://images.unsplash.com/photo-1480714378408-67cf0d13bc1f?auto=format&fit=crop&w=1800&q=85',year:2024,status:'Finished',score:8.7,type:'Movie',episodes:1,featured:false,categories:[1,2,6],views:9801,createdAt:'2026-08-31T00:00:00.000Z'},
 {id:5,title:'Beyond The Abyss',slug:'beyond-the-abyss',synopsis:'Uma equipe de exploradores desce cada vez mais fundo em um mundo onde cada nível revela uma nova regra.',cover:'https://images.unsplash.com/photo-1472214103451-9374bd1c798e?auto=format&fit=crop&w=800&q=85',banner:'https://images.unsplash.com/photo-1470770841072-f978cf4d019e?auto=format&fit=crop&w=1800&q=85',year:2025,status:'Ongoing',score:8.8,type:'TV',episodes:18,featured:false,categories:[2,4,8],views:8704,createdAt:'2026-08-29T00:00:00.000Z'},
 {id:6,title:'Skyline Hearts',slug:'skyline-hearts',synopsis:'Dois estudantes descobrem que seus sonhos estão conectados a uma cidade que só existe depois da meia-noite.',cover:'https://images.unsplash.com/photo-1516589178581-6cd7833ae3b2?auto=format&fit=crop&w=800&q=85',banner:'https://images.unsplash.com/photo-1482192596544-9eb780fc7f66?auto=format&fit=crop&w=1800&q=85',year:2026,status:'Ongoing',score:8.6,type:'TV',episodes:8,featured:false,categories:[3,5],views:7118,createdAt:'2026-08-28T00:00:00.000Z'}
];
const demoEpisodes=[];
for(const a of demoAnime){ for(let i=1;i<=Math.min(a.episodes,6);i++) demoEpisodes.push({id:Date.now()+a.id*100+i,animeId:a.id,season:1,episode:i,title:`${['O despertar','Um novo caminho','A primeira missão','Sinais no céu','A grande virada','Antes do amanhecer'][i-1]||`Episódio ${i}`}`,duration:'24 min',language:'Legendado',streamUrl:'',thumbnail:a.cover}); }
...
function ensure(){if(!fs.existsSync(path.dirname(DB)))fs.mkdirSync(path.dirname(DB),{recursive:true});if(!fs.existsSync(DB)){const d=structuredClone(initial);d.anime=demoAnime;d.episodes=demoEpisodes;d.sections=initialSections;write(d)}const d=read();let changed=false;if(!d.settings)d.settings=structuredClone(initial.settings); if(!Array.isArray(d.settings.externalApis)) { d.settings.externalApis=[]; changed=true; }if(!d.categories?.length){d.categories=structuredClone(initial.categories);changed=true}if(!d.anime?.length){d.anime=demoAnime;changed=true}if(!d.episodes?.length){d.episodes=demoEpisodes;changed=true}if(!d.sections?.length){d.sections=initialSections;changed=true}if(!d.users)d.users=[];if(!d.logs)d.logs=[];if(!d.comments)d.comments=[];if(!d.users.some(u=>u.email===ADMIN_EMAIL)){d.users.push({id:1,email:ADMIN_EMAIL,passwordHash:hashPassword(ADMIN_PASSWORD),role:'admin',blocked:false,createdAt:now(),favorites:[],history:[],lastLogin:null});changed=true}if(changed)write(d)}
...
async function fetchExternalJson(source,page=1){
  ...
  const url=applyTemplate(source.url,page,limit);
  const headers={'accept':'application/json',...(source.headers||{})};
  const init={method:source.method||'GET',headers};
  if(init.method==='POST'){init.body=JSON.stringify(source.body||{});headers['content-type']=headers['content-type']||'application/json'}
  const r=await fetch(url,init);if(!r.ok)throw new Error(`${source.name}: HTTP ${r.status}`);return r.json();
}
async function syncExternalApis(d, progress=()=>{}){
  ...
  for(let si=0;si<sources.length;si++){
    ...
    progress({currentSource:si+1,totalSources:sources.length,currentPage:0,totalPages:source.pagination?.enabled?Math.max(1,Math.min(100,Number(source.pagination.maxPages)||10)):1,message:`Sincronizando ${source.name}`});
    result.sources++;
    try{
      const pages=source.pagination?.enabled?Math.max(1,Math.min(100,Number(source.pagination.maxPages)||10)):1;
      ...
      for(let pi=0;pi<pages;pi++){
        ...
        for(const m of items){
          ...
          const externalId=String(firstValue(m,source.mapping.id,''));const slug=slugify(firstValue(m,source.mapping.slug,title));
          ...
          const tagsRaw=firstValue(m,source.mapping.tags,[]);const tags=Array.isArray(tagsRaw)?tagsRaw.map(x=>typeof x==='object'?String(x.name||x.title||''):String(x)).filter(Boolean).slice(0,30):String(tagsRaw||'').split(',').map(x=>x.trim()).filter(Boolean).slice(0,30);
          ...
          const upsertEpisode=(raw,n)=>{const num=Number(firstValue(raw,['episode','number','episodeNumber'],n))||n;if(!num)return;const exists=d.episodes.find(e=>e.animeId===a.id&&e.season===(Number(firstValue(raw,['season','seasonNumber'],1))||1)&&e.episode===num);const ep={id:exists?.id||Date.now()+Math.floor(Math.random()*1000000),animeId:a.id,season:Number(firstValue(raw,['season','seasonNumber'],1))||1,episode:num,title:String(firstValue(raw,['title','name'],`Episódio ${num}`)),duration:String(firstValue(raw,['duration','runtime'],a.duration?`${a.duration} min`:'24 min')),language:String(firstValue(raw,['language','lang'],'')),streamUrl:cleanUrl(firstValue(raw,['streamUrl','video','url'],'')),watchUrl:cleanUrl(firstValue(raw,['watchUrl','externalUrl'],'')),thumbnail:cleanUrl(firstValue(raw,['thumbnail','image','cover'],a.cover)),createdAt:exists?.createdAt||now()};if(exists)Object.assign(exists,ep);else{d.episodes.push(ep);result.episodesCreated++}};
          episodeItems.forEach((e,i)=>upsertEpisode(e,i+1));
          if(!episodeItems.length&&source.episodeUrlTemplate&&externalId){const epUrl=source.episodeUrlTemplate.replaceAll('{id}',encodeURIComponent(externalId));const epSrc={...source,url:epUrl,itemsPath:source.episodeItemsPath,pagination:{enabled:false}};const ej=await fetchExternalJson(epSrc,1);const eis=getPath(ej,source.episodeItemsPath);if(Array.isArray(eis))eis.forEach((e,i)=>upsertEpisode(e,i+1));}
          if(!episodeItems.length&&epCount>0){for(let n=1;n<=epCount;n++){if(!d.episodes.some(e=>e.animeId===a.id&&e.season===1&&e.episode===n)){d.episodes.push({id:Date.now()+Math.floor(Math.random()*1000000),animeId:a.id,season:1,episode:n,title:`Episódio ${n}`,duration:'',language:'',streamUrl:'',watchUrl:'',thumbnail:a.cover,createdAt:now()});result.episodesCreated++}}}
        }
        if(!source.pagination?.enabled)break;
        const hasNext=source.nextPath?Boolean(getPath(j,source.nextPath)):items.length>=limit;if(!hasNext)break;
      }
...
async function startExternalSync(d){
  if(syncRunning) return {job:syncJob,alreadyRunning:true};
  syncRunning=true;
  syncJob={id:crypto.randomUUID(),status:'running',startedAt:now(),finishedAt:null,totalSources:(d.settings?.externalApis||[]).filter(x=>x.enabled!==false&&x.url).length,currentSource:0,currentPage:0,totalPages:0,discovered:0,updated:0,episodesCreated:0,errors:[],message:'Iniciando sincronização…'};
  const jobId=syncJob.id;
  setImmediate(async()=>{
    try{
      const result=await syncExternalApis(d,p=>{if(syncJob.id===jobId) Object.assign(syncJob,p);});
      d.settings.externalApisSyncedAt=now();
      d.settings.externalApisLastResult=result;
      write(d);
      Object.assign(syncJob,{status:'completed',finishedAt:now(),currentSource:syncJob.totalSources,currentPage:syncJob.totalPages,discovered:result.discovered,updated:result.updated,episodesCreated:result.episodesCreated,errors:result.errors,message:result.errors.length?'Concluído com avisos':'Sincronização concluída'});
    }catch(e){
      Object.assign(syncJob,{status:'failed',finishedAt:now(),errors:[...syncJob.errors,{source:'sistema',error:e.message}],message:e.message||'Falha na sincronização'});
    }finally{syncRunning=false;}
  });
  return {job:syncJob};
}

async function syncAniList(d){
  // Importa dados reais e atualizados do AniList. O catálogo fica no banco local
  // para que busca, favoritos, histórico e páginas de detalhes continuem funcionando.
  const q=`query ($page:Int!,$perPage:Int!,$sort:[MediaSort!],$year:Int,$status:MediaStatus){
    Page(page:$page,perPage:$perPage){
      pageInfo{hasNextPage}
      media(type:ANIME,isAdult:false,sort:$sort,seasonYear:$year,status:$status){
        id title{romaji english native}
        description(asHtml:false)
        coverImage{extraLarge large}
        bannerImage
        siteUrl
        year seasonYear:year status averageScore genres
        episodes duration format
  ...
  const fetchPage=async(variables)=>{
  ...
  let updated=0,discovered=0,episodesCreated=0,pages=0;
  const seen=new Set();
  const year=new Date().getUTCFullYear();

  const importMedia=async(media)=>{
    for(const m of media){
      ...
      let a=d.anime.find(x=>x.anilistId===m.id||x.slug===slug);
      const tags=[...(m.genres||[])].map(String).slice(0,20);
      ...
      const status=statusMap[m.status]||m.status||'Ongoing';
      ...
      const stream=(m.streamingEpisodes||[]).find(x=>x?.url)?.url||'';
      const cover=cleanUrl(m.coverImage?.extraLarge||m.coverImage?.large);
      const banner=cleanUrl(m.bannerImage);
      const synopsis=String(m.description||'').replace(/<[^>]+>/g,'').slice(0,3000);

      if(!a){
        const id=Math.max(0,...d.anime.map(x=>Number(x.id)||0))+1;
        a={id,title,slug,synopsis,cover,banner,year:m.year||null,status,score,type,
          episodes:Number(m.episodes||0),featured:score>=8.5,categories:[],views:0,
          tags,anilistId:m.id,anilistUrl:cleanUrl(m.siteUrl),nextAiring:next,
          duration:Number(m.duration||24),createdAt:now(),updatedAt:now()};
        // Reaproveita as categorias existentes quando houver correspondência de gênero.
        a.categories=d.categories.filter(c=>tags.some(t=>slugify(t)===c.slug)).map(c=>c.id);
        d.anime.push(a); discovered++;
      }else{
        Object.assign(a,{title,slug,synopsis,cover:cover||a.cover,banner:banner||a.banner,
          year:m.year||a.year,status,score,type,episodes:Number(m.episodes||a.episodes||0),
          featured:score>=8.5,tags,anilistId:m.id,anilistUrl:cleanUrl(m.siteUrl),
          nextAiring:next,duration:Number(m.duration||a.duration||24),updatedAt:now()});
        a.categories=d.categories.filter(c=>tags.some(t=>slugify(t)===c.slug)).map(c=>c.id);
        updated++;
      }

      // Cria somente a numeração real de episódios informada pelo AniList.
      // Não inventa vídeo: streamUrl fica vazio até existir uma fonte licenciada configurada.
      const count=Number(m.episodes||0);
      if(count>0){
        const existing=new Set(d.episodes.filter(e=>e.animeId===a.id).map(e=>`${e.season}:${e.episode}`));
        const streams=Array.isArray(m.streamingEpisodes)?m.streamingEpisodes:[];
        for(let n=1;n<=count;n++){
          const key=`1:${n}`;
          if(existing.has(key))continue;
          const legal=streams.find(x=>Number(String(x?.title||'').match(/(\d+)/)?.[1])===n)?.url||'';
          d.episodes.push({
            id:Date.now()+Math.floor(Math.random()*1000000),
            animeId:a.id,season:1,episode:n,title:`Episódio ${n}`,
            duration:`${Number(m.duration||24)} min`,language:'Legendado',
            streamUrl:'',watchUrl:cleanUrl(legal),thumbnail:a.cover,createdAt:now()
          });
          episodesCreated++;
        }
      }
  ...
  try{
    // 8 páginas do ano atual + 8 páginas populares: até ~800 títulos reais,
    // deduplicados. O catálogo continua podendo ser atualizado pelo mesmo botão.
    for(let page=1;page<=8;page++){
      const data=await fetchPage({page,perPage:50,sort:['UPDATED_AT_DESC'],year,status:'RELEASING'});
      if(page===1 && !d.anime.some(x=>x.anilistId) && (data?.media||[]).length){
        d.anime=[]; d.episodes=[];
      }
      pages++;
    ...
    for(let page=1;page<=8;page++){
      const data=await fetchPage({page,perPage:50,sort:['POPULARITY_DESC']});
      pages++;
      await importMedia(data?.media||[]);
      if(!data?.pageInfo?.hasNextPage)break;
    }
  }catch(e){
    return {updated,discovered,episodesCreated,pages,error:e.message,at:now()};
  }
  return {updated,discovered,episodesCreated,pages,at:now()};
}
...
setTimeout(async()=>{
  try{
    const d=read();
    const last=Date.parse(d.settings?.catalogSyncedAt||'');
    if(!Number.isFinite(last) || Date.now()-last>6*60*60*1000){
      const result=await syncAniList(d);
      if(!result.error){
        d.settings.catalogSyncedAt=now();
        d.settings.announcement=`Catálogo real atualizado: ${result.discovered} novos animes e ${result.episodesCreated} episódios.`;
        write(d);
      }
      const ext=await syncExternalApis(d);
      if(ext.sources){ d.settings.externalApisSyncedAt=now(); d.settings.externalApisLastResult=ext; write(d); }
    }
  }catch{}
},1500);
setInterval(async()=>{try{const d=read();const last=Date.parse(d.settings?.externalApisSyncedAt||'');if(Array.isArray(d.settings?.externalApis)&&d.settings.externalApis.length&&(!Number.isFinite(last)||Date.now()-last>6*60*60*1000)&&!syncRunning){await startExternalSync(d)}}catch{}} ,6*60*60*1000);
const server=http.createServer(async(req,res)=>{
  try{
    ...
    if(p.startsWith('/api/')){
      ...
      if(p==='/api/admin/external-apis/sync'&&method==='POST'){const u=currentUser(req,d);if(!u||u.role!=='admin')return fail(res,403,'Acesso negado');const r=await startExternalSync(d);audit(d,u,'START_SYNC_EXTERNAL_APIS','external-apis',{jobId:r.job?.id,alreadyRunning:!!r.alreadyRunning});return ok(res,r)}
      ...
      if(p==='/api/admin/anilist/sync'&&method==='POST'){const result=await syncAniList(d);write(d);audit(d,u,'SYNC_ANILIST',String(result.updated));return ok(res,result);}
...
server.listen(PORT,()=>console.log(`AnimeDragon em http://localhost:${PORT}`));
